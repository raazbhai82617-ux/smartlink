...
body {
  ...
  background:
    ...
    radial-gradient(900px 500px at 100% 0%, rgba(99,102,241,.08), transparent 50%),
    radial-gradient(700px 400px at 50% 100%, rgba(212,175,55,.05), transparent 55%),
    var(--bg);
...
.menu .gold { color: var(--gold) !important; }
.menu .gold:hover { background: var(--gold-dim) !important; }
...
.hero h1 {
  ...
  line-height: .98; margin: 14px 0 18px;
  background: linear-gradient(180deg, #fff8e8 20%, var(--gold-soft));
  -webkit-background-clip: text; background-clip: text;
  -webkit-text-fill-color: transparent;
}
.hero p { color: var(--muted); max-width: 540px; font-size: 16px; line-height: 1.7; }

/* Forms */
form.row { display: flex; gap: 12px; margin-top: 36px; flex-wrap: wrap; }
form.stack { display: flex; flex-direction: column; gap: 16px; margin-top: 28px; max-width: 440px; }
form.stack label {
  font-size: 11px; letter-spacing: .14em; text-transform: uppercase;
  color: var(--muted); display: block; margin-bottom: 8px; font-weight: 600;
}
input[type="url"], input[type="email"], input[type="password"], input[type="text"], input[type="search"] {
  width: 100%;
...
input::placeholder { color: #5c574f; }
form.row input[type="url"] { flex: 1; min-width: 220px; }

/* Buttons */
.btn {
  display: inline-flex; align-items: center; justify-content: center; gap: 8px;
  background: linear-gradient(135deg, #e4c35a, #c49a28);
  color: #14110a; border: 0;
...
.btn:hover {
  ...
  box-shadow: 0 14px 36px rgba(212,175,55,.32);
...
.stat b {
  display: block;
  font-family: var(--display);
  font-size: 40px; line-height: 1; color: var(--gold-soft);
}
.stat span {
...
.card-h h2 { font-size: 28px; }
.card-h p { color: var(--muted); font-size: 14px; margin-top: 6px; }
...
.gold { color: var(--gold-soft); font-weight: 700; }
.muted { color: var(--muted); }
.err {
  color: var(--danger); font-size: 14px; margin-top: 12px;
...
.grid3 { display: grid; gap: 14px; grid-template-columns: 1fr; margin: 24px 0; }
.panel {
...
.modal {
  width: min(440px, 100%);
  background: linear-gradient(180deg, #161821, #0f1016);
  border: 1px solid var(--line-strong);
  border-radius: 22px; padding: 28px;
  box-shadow: 0 30px 100px rgba(0,0,0,.55);
}
.tg {
  display: block; margin-top: 18px; padding: 20px;
  border-radius: 16px; text-decoration: none;
  background: linear-gradient(145deg, rgba(212,175,55,.12), rgba(212,175,55,.03));
  border: 1px solid var(--line-strong);
  transition: .2s ease;
}
.tg:hover { transform: translateY(-2px); border-color: rgba(212,175,55,.4); }
.tg b { display: block; font-family: var(--display); font-size: 30px; }
.tg span { color: var(--gold); font-weight: 600; }

.auth-box {
  max-width: 460px; margin: 0 auto;
  padding: 8px;
}
.auth-box .card, .auth-shell {
  padding: 8px 0;
}
...
<header>
  <div class="wrap nav">
    <a class="brand" href="/">
      ...
      <span><b>SmartLink</b><small>Premium Shortener</small></span>
    </a>
    <nav class="menu">
      {auth_links}
...
<footer>
  <div class="wrap foot">
    <span>SmartLink · Crafted for precision tracking</span>
...
def home():
    body = """
    <section class="hero">
      <p class="kicker">Private · Precise · Premium</p>
      <h1>Shorten links with cinematic clarity.</h1>
      <p>Create elegant short links and track every click — country, device, OS, browser, date and time — in a private workspace only you control.</p>
...
def dashboard():
    ...
    body = f"""
    <p class="kicker">Your private command center</p>
    <h1>Dashboard</h1>
    <div class="stats">
    ...
    <div class="card">
      <div class="card-h">
        ...
        <p>Private by design. Copy, analyze, or delete your links anytime.</p>
      </div>
