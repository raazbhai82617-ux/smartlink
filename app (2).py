...
def current_user():
    ...
    try:
        with get_db() as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT id, name, email, is_admin, COALESCE(is_blocked, 0) FROM users WHERE id = ?",
                (uid,),
            )
            row = cur.fetchone()
        if not row:
    ...
    if request.method == "POST":
        ...
        try:
            with get_db() as conn:
                cur = conn.cursor()
                cur.execute(
                    "SELECT id, password_hash, COALESCE(is_blocked, 0) FROM users WHERE email = ?",
                    (email,),
                )
                row = cur.fetchone()
    ...
    try:
        ...
        <div class="card" style="margin-top:24px;padding:24px">
          ...
          <p style="margin:10px 0 18px;word-break:break-all">
            <a class="gold" href="{safe_url}">{safe_url}</a>
          </p>
...
def dashboard():
    ...
    <div class="card">
      <div class="card-h">
        ...
        <p>Other users cannot see these links.</p>
      </div>
      <div class="table-wrap">
        <table>
          <thead>
            <tr><th>Short link</th><th>Destination</th><th>Clicks</th><th>Created</th><th>Details</th></tr>
          </thead>
          <tbody>{rows}</tbody>
        </table>
      </div>
...
def admin_panel():
    try:
        with get_db() as conn:
            ...
            total_clicks = cur.fetchone()[0]
            cur.execute("""
                SELECT u.id, u.name, u.email, u.is_admin, COALESCE(u.is_blocked,0), u.created_at,
...
def admin_block_user(user_id):
    ...
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute(
            "UPDATE users SET is_blocked = 1 WHERE id = ? AND COALESCE(is_admin,0) = 0",
            (user_id,),
        )
        conn.commit()
    return redirect("/admin")
...
def redirect_short_link(short_code):
    reserved = {
        ...
        "support", "signup", "signin", "signout", "admin",
    }
    ...
    # Tracking is best-effort — never block redirect (Facebook safe)
    try:
