...
def admin_panel():
    ...
    for uid, name, email, is_admin, created, link_count, click_count in users:
        ...
        <tr>
          ...
          <td>{int(link_count)}</td>
          <td>{int(click_count)}</td>
          <td class="muted">{format_dt(created)}</td>
        </tr>
        """
    if not user_rows:
    ...
    for short, url, clicks, created, email in links:
        ...
        link_rows += f"""
        <tr>
          <td><a class="gold" href="/analytics/{sc}">/{sc}</a></td>
          <td class="muted">{display}</td>
          <td>{int(clicks)}</td>
          <td>{owner}</td>
          <td class="muted">{format_dt(created)}</td>
        </tr>
    ...
    <div class="stats">
      ...
      <div class="stat"><b>{total_clicks}</b><span>All clicks</span></div>
    </div>
    <div class="card" style="margin-bottom:24px">
      <div class="card-h">
        <h2>Users</h2>
        <p>Who signed up, when, and their totals</p>
      </div>
      <div class="table-wrap">
        <table>
          <thead>
            <tr>
              ...
              <th>Links</th><th>Clicks</th><th>Joined</th>
            </tr>
          </thead>
